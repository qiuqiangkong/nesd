...
def train(args) -> None:
    ...
    for step, data in enumerate(tqdm(train_dataloader)):
        ...
        if step % configs["train"]["test_every_n_steps"] == 0:
            ...
            #     wandb.log(
            #         data={
            #             "train_sdr": train_stats["sdr"], 
            #             "train_sdri": train_stats["sdri"], 
            #             "test_sdr": test_stats["sdr"], 
            #             "test_sdri": test_stats["sdri"], 
            #             "test_sdr_ema": test_stats_ema["sdr"],
            #             "test_sdri_ema": test_stats_ema["sdri"]
            #         },
            #         step=step
            #     )
...
def get_data_transform(configs: dict):
    ...
    if name == "Render":
        from nesd.data_transforms.render import Render
        return Render(configs["data_transform"], sr)
...
def get_model(
    ...
    if name == "DNN":
        from nesd.models.dnn import DNN
        model = DNN(**configs["model"])

    elif name == "DNN2":
        from nesd.models.dnn2 import DNN2
        model = DNN2(**configs["model"])

    elif name == "DNN3":
        from nesd.models.dnn3 import DNN3
        model = DNN3(**configs["model"])

    elif name == "DNN4":
        from nesd.models.dnn4 import DNN4
        model = DNN4(**configs["model"])

    elif name == "DNN4b":
        from nesd.models.dnn4b import DNN4b
        model = DNN4b(**configs["model"])

    elif name == "DNN5":
        from nesd.models.dnn5 import DNN5
        model = DNN5(**configs["model"])
