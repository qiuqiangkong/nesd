...
if select == "1":
    audio_paths = [Path(dataset_dir, "mic_dev", "fold1_room1_mix026_ov2.wav")]

    panaromas_dir = Path(workspace, "results/dcase2020_task3/panaroma")
    panaroma_paths = [Path(workspace, "results/dcase2020_task3/panaroma/fold1_room1_mix026_ov2.pkl")]

    pred_csvs_dir = Path(workspace, "results/dcase2020_task3/pred_csvs")

    gt_csv_path = Path(dataset_dir, "metadata_dev", "fold1_room1_mix026_ov2.csv")
...
def write_loc_csv(args):
    ...
    for panaroma_path in panaroma_paths:
        ...
        # Write result to csv
        class_ids = [[4] * len(locs) for locs in locss]
