...
class DCASE2020Task3:
    def __init__(
        ...
        if split == "train":
            self.audio_paths = [path for path in audio_paths if re.match("(.*)fold[1-4](.*)", str(path))]

        elif split == "test":
            self.audio_paths = [path for path in audio_paths if re.match("(.*)fold[5-6](.*)", str(path))]
