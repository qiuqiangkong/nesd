...
from nesd.old_models.base import init_layer, init_bn, init_gru, Base, cart2sph_torch, interpolate, get_position_encodings, PositionalEncoder, LookDirectionEncoder, LookDepthEncoder
from nesd.old_models.models01 import ConvBlockRes
...
class Model02(nn.Module, Base):
    ...
    def forward(
        ...
        mic_look_directions = data_dict["mic_orientations"]
        mic_signals = data_dict["mic_wavs"]
        agent_positions = data_dict["agent_positions"]
        agent_look_directions = data_dict["agent_look_at_directions"]
        ...
        if True:
            ...
            if self.training:
                output_dict['agent_look_at_direction_has_source'] = agent_look_directions_has_source[:, 0 : 20, :]
            else:
                output_dict['agent_look_at_direction_has_source'] = agent_look_directions_has_source
...
class Model02_sep_linear(nn.Module, Base):
    def __init__(self, 
        mics_num: int, 
        # classes_num: int, 
        # do_localization: bool, 
        ...
        super(Model02_sep_linear, self).__init__() 
...
class Model02_Cla(nn.Module, Base):
    def __init__(self, 
        mics_num: int, 
        # classes_num: int, 
        # do_localization: bool, 
        ...
        super(Model02_Cla, self).__init__() 
        ...
        if True:
            self.cla_fc_block1 = ConvBlock(
                in_channels=1536, 
                out_channels=1024, 
                kernel_size=(1, 1)
            )

            self.cla_fc_block2 = ConvBlock(
                in_channels=1024, 
                out_channels=1024,
                kernel_size=(1, 1),
            )

            self.cla_fc_final = nn.Linear(1024, 20, bias=True)
    ...
    def forward(
        ...
        if True:
            ...
            x = self.cla_fc_block1(x)
            # (bs * agents_num, 1024, T, 1)

            x = self.cla_fc_block2(x)
            # (bs * agents_num, 1024, T, 1)

            x = rearrange(x.squeeze(), '(b n) c t -> (b n) t c', b=batch_size)
            # (bs * agents_num, T, 1024)

            x = torch.sigmoid(self.cla_fc_final(x))
            # (bs * agents_num, T=38, C=1)

            x = x.repeat_interleave(repeats=self.time_downsample_ratio, dim=1)[:, 0 : frames_num, :]
            # (bs * agents_num, T=301, C=1)

            agent_look_directions_cla = rearrange(x, '(b n) t c -> b n t c', b=batch_size)

            output_dict['agent_sed'] = agent_look_directions_cla
...
'''
class Model02_depth_sep(nn.Module, Base):
    def __init__(self, 
        mics_num: int, 
        # classes_num: int, 
        # do_localization: bool, 
        ...
        super(Model02_depth_sep, self).__init__() 
...
class Model02_depth_sepecho(nn.Module, Base):
    def __init__(self, 
        mics_num: int, 
        # classes_num: int, 
        # do_localization: bool, 
        ...
        super(Model02_depth_sepecho, self).__init__() 
