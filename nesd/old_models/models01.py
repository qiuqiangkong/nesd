...
from nesd.old_models.base import init_layer, init_bn, init_gru, Base, cart2sph_torch, interpolate, get_position_encodings, PositionalEncoder, LookDirectionEncoder, DepthEncoder
...
class Model01_4GroupMics(nn.Module, Base):
    def __init__(self, 
        microphones_num: int, 
        classes_num: int, 
        do_localization: bool, 
        ...
        super(Model01_4GroupMics, self).__init__() 
        ...
        self.mic_position_fc = nn.Linear(
            in_features=microphones_num * (positional_embedding_factor * 6) * 4, 
            out_features=128, 
            bias=True
        )

        self.mic_look_direction_fc = nn.Linear(
            in_features=microphones_num * (positional_embedding_factor * 4) * 4, 
            out_features=128, 
            bias=True
        )
    ...
    def forward(
        ...
        # soundfile.write(file='_zz.wav', data=total_waveform[0, 0].data.cpu().numpy(), samplerate=24000)
        ...
        for group in range(4):
            for i in range(1, 4):
                for j in range(0, i):
                    _delta_cos = mic_cos[:, 4 * group + i, :, :] * mic_cos[:, 4 * group + j, :, :] + mic_sin[:, 4 * group + i, :, :] * mic_sin[:, 4 * group + j, :, :]

                    _delta_sin = mic_sin[:, 4 * group + i, :, :] * mic_cos[:, 4 * group + j, :, :] - mic_cos[:, 4 * group + i, :, :] * mic_sin[:, 4 * group + j, :, :]
...
class Model01_4GroupMicsDepth(nn.Module, Base):
    def __init__(self, 
        microphones_num: int, 
        classes_num: int, 
        do_localization: bool, 
        ...
        super(Model01_4GroupMicsDepth, self).__init__() 
        ...
        self.look_depth_encoder = DepthEncoder(
            factor=positional_embedding_factor
        )
    ...
    def convert_look_depth_to_embedding(self, look_depth):
        r"""Convert look direction to embedding.
        ...
        Returns:
            position_emb: (batch_size, positions_num, frames_num, emb_size)
        """
        look_depth_emb = self.look_depth_encoder(depth=look_depth)
    ...
    def forward(
        ...
        agent_position = data_dict['agent_position']    # (bs, agents_num, frames_num, 3)
        agent_look_direction = data_dict['agent_look_direction']    # (bs, agents_num, frames_num, 3)
        agent_look_depth = data_dict['agent_look_depth']

        max_agents_contain_waveform = data_dict['max_agents_contain_waveform']
        max_agents_contain_depth = data_dict['max_agents_contain_depth']
        ...
        agent_look_depth_emb = self.convert_look_depth_to_embedding(
        ...
        agent_position_emb = F.pad(agent_position_emb, pad=(0, 0, 0, pad_len))
        agent_look_direction_emb = F.pad(agent_look_direction_emb, pad=(0, 0, 0, pad_len))
        agent_look_depth_emb = F.pad(agent_look_depth_emb, pad=(0, 0, 0, pad_len))
        ...
        agent_look_depth_feature = F.leaky_relu_(
            self.agent_look_depth_fc(agent_look_depth_emb)
        )  # (bs, agents_num, T, 128)
        ...
        # Concatenate mic features and agent features.
        shared_feature = torch.cat((mic_feature, agent_position_feature, 
            agent_look_direction_feature, tmp), dim=-1)
        # (bs, agents_num, T=38, 1536)
        ...
        if self.do_depth:
            ...
            agent_exist_source = rearrange(x.squeeze(), '(b n) t -> b n t', b=batch_size)

            output_dict['agent_exist_source'] = agent_exist_source

        return output_dict


# class Model01_classwise(nn.Module, Base):
#     def __init__(self, 
#         microphones_num: int, 
#         classes_num: int, 
#         do_localization: bool, 
...
#         do_separation: bool,
#     ):
#         super(Model01_classwise, self).__init__() 
...
#         if self.do_sed:
#             self.sed_fc_block1 = ConvBlock(
#                 in_channels=1536, 
#                 out_channels=1024, 
#                 kernel_size=(1, 1)
#             )

#             self.sed_fc_block2 = ConvBlock(
#                 in_channels=1024, 
#                 out_channels=1024,
#                 kernel_size=(1, 1),
#             )

#             self.sed_fc_final = nn.Linear(1024, classes_num, bias=True)
...
#             x = self.sed_fc_block1(x)
#             # (bs * agents_num, 1024, T, 1)

#             x = self.sed_fc_block2(x)
#             # (bs * agents_num, 1024, T, 1)

#             x = rearrange(x.squeeze(), '(b n) c t -> (b n) t c', b=batch_size)
#             # (bs * agents_num, T, 1024)

#             x = torch.sigmoid(self.sed_fc_final(x))
#             # (bs * agents_num, T=38, classes_num)

#             x = x.repeat_interleave(repeats=self.time_downsample_ratio, dim=1)[:, 0 : frames_num, :]
#             # (bs * agents_num, T=301, classes_num)

#             agent_see_source_classwise = rearrange(x, '(b n) t c -> b n t c', b=batch_size)

#             output_dict['agent_see_source_classwise'] = agent_see_source_classwise
...
class Model01_Rnn(nn.Module, Base):
    def __init__(self, 
        microphones_num: int, 
        classes_num: int, 
        do_localization: bool, 
        ...
        super(Model01_Rnn, self).__init__() 
...
class Model01_Rnn_classwise(nn.Module, Base):
    def __init__(self, 
        microphones_num: int, 
        classes_num: int, 
        do_localization: bool, 
        ...
        super(Model01_Rnn_classwise, self).__init__() 
...
class EncoderFc(nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            ...
            encoder: (batch_size, output_feature_maps, time_steps, freq_bins)
        """
        output = F.leaky_relu_(self.fc(input_tensor))
        return output


class ModelSepRnn(nn.Module, Base):
    def __init__(self, 
        microphones_num: int, 
        classes_num: int, 
        do_localization: bool, 
        ...
        super(ModelSepRnn, self).__init__() 
        ...
        self.mic_signal_encoder_block1 = EncoderFc(
            in_channels=16384, 
            out_channels=1024, 
        )

        self.mic_signal_encoder_block2 = EncoderFc(
            in_channels=8192, 
            out_channels=2048, 
        )

        self.mic_signal_encoder_block3 = EncoderFc(
            in_channels=2048, 
            out_channels=2048, 
        )
        ...
        if self.do_separation:
            ...
            self.sep_decoder_gru = nn.GRU(input_size=2048, hidden_size=1024, num_layers=3, bias=True, batch_first=True, dropout=0., bidirectional=True)

            self.sep_fc_final = nn.Linear(
                in_features=2048, 
                out_features=256 * 8 * 3,  
                bias=True
            )
    ...
    def init_weights(self):
        ...
        if self.do_separation:
            init_layer(self.sep_fc_reshape)
            init_gru(self.sep_decoder_gru)
            init_layer(self.sep_fc_final)
    ...
    def forward(
        ...
        # 1) Transform mic signal.
        x = rearrange(x, 'b c t f -> b t c f').flatten(2)
        x = self.mic_signal_encoder_block1(x)

        _B, _T, _C = x.shape
        x = x.reshape(_B, 38, 8, _C).flatten(2)

        x = self.mic_signal_encoder_block2(x)
        x = self.mic_signal_encoder_block3(x)
        # enc3_pool: (bs, 128, T=38, F=32)

        mic_signal_feature = F.leaky_relu_(
            self.mic_signal_fc_reshape(x), negative_slope=0.01,
        )  # shape: (bs, T, 1024)
        ...
        if do_separation:
            ...
            x, _ = self.sep_decoder_gru(x)

            x = self.sep_fc_final(x)
            # (bs * n, C=3, T=304, F=256)

            _B, _T, _C = x.shape
            x = x.reshape(_B, _T, 8, 256, 3)
            x = rearrange(x, 'b t t2 f c -> b c (t t2) f')
