...
from nesd.models.base import PositionEncoder, OrientationEncoder, DistanceEncoder, AngleEncoder, OrientationEncoder2, AngleEncoder2, OrientationEncoder3, AngleEncoder3
...
class MLP_nll_loss(nn.Module):
    def __init__(self, in_channels, hid_channels, out_channels):
        super(MLP_nll_loss, self).__init__()
        self.fc1 = nn.Linear(in_channels, hid_channels)
        self.fc2 = nn.Linear(hid_channels, hid_channels)
        self.fc3 = nn.Linear(hid_channels, hid_channels)
        self.fc4 = nn.Linear(hid_channels, out_channels)
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu_(self.fc2(x))
        x = F.leaky_relu_(self.fc3(x))
        output = torch.log_softmax(self.fc4(x), dim=-1)
        return output
...
class NeSD4(Fourier):
    ...
    def forward(
        ...
        # _ = self.orien_encoder(agent_look_at_directions)
...
class NeSD7(Fourier):
    def __init__(self, 
        ...
        super(NeSD7, self).__init__(n_fft=n_fft, hop_length=hop_length) 
...
class NeSD7_srcs_num(Fourier):
    def __init__(self, 
        ...
        super(NeSD7_srcs_num, self).__init__(n_fft=n_fft, hop_length=hop_length) 
...
class NeSD8(Fourier):
    def __init__(self, 
        ...
        super(NeSD8, self).__init__(n_fft=n_fft, hop_length=hop_length) 
        ...
        self.pos_encoder = PositionEncoder(size=pos_enc_size)
        self.orien_encoder = OrientationEncoder2(size=orien_enc_size)
        self.dist_encoder = DistanceEncoder(size=dist_enc_size)
        self.angle_encoder = AngleEncoder2(size=angle_enc_size)
...
class NeSD9(Fourier):
    def __init__(self, 
        ...
        super(NeSD9, self).__init__(n_fft=n_fft, hop_length=hop_length) 
        ...
        self.pos_encoder = PositionEncoder(size=pos_enc_size)
        self.orien_encoder = OrientationEncoder3(size=orien_enc_size)
        self.dist_encoder = DistanceEncoder(size=dist_enc_size)
        self.angle_encoder = AngleEncoder3(size=angle_enc_size)
        ...
        self.mic_orien_fc = nn.Linear(
            in_features=mics_num * orien_enc_size, 
            out_features=128, 
            bias=True
        )
