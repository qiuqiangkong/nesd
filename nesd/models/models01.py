...
class Model01(nn.Module, Base):
    def __init__(self, microphones_num, classes_num, do_separate):
        ...
        self.look_direction_encoder = PositionalEncoder(factor=positional_embedding_factor)
        self.position_encoder = PositionalEncoderRoomXYZ(factor=positional_embedding_factor)
        ...
        self.mic_signal_conv_block1 = EncoderBlockRes1B(
            in_channels=64, 
            out_channels=32, 
            kernel_size=(3, 3), 
            downsample=(2, 2), 
        ...
        self.mic_signal_conv_block2 = EncoderBlockRes1B(
            in_channels=32, 
            out_channels=64, 
            kernel_size=(3, 3), 
            downsample=(2, 2), 
        ...
        self.mic_signal_conv_block3 = EncoderBlockRes1B(
            in_channels=64, 
            out_channels=128, 
            kernel_size=(3, 3), 
            downsample=(2, 2), 
        ...
        self.mic_signal_fc1 = nn.Linear(in_features=32 * 128, out_features=1024)
    ...
    def init_weights(self):
        ...
        init_layer(self.mic_signal_fc1)
    ...
    def convert_look_direction_to_emb(self, look_direction):
        r"""Convert look direction to embedding.

        Args:
            look_direction: (bs, directions_num, frames_num, 3)

        Returns:
            look_direction_emb: (bs, directions_num, frames_num, emb_size)
        """
    ...
    def convert_position_to_emb(self, position):
        r"""Convert look direction to embedding.

        Args:
            position: (bs, positions_num, frames_num, 3)

        Returns:
            position_emb: (bs, positions_num, frames_num, emb_size)
        """
    ...
    def forward(self, data_dict):
        ...
        mic_position_emb = self.convert_position_to_emb(position=mic_position)
        # shape: (bs, mics_num, frames_num, emb_size)

        mic_look_direction_emb = self.convert_look_direction_to_emb(
            look_direction=mic_look_direction,
        )   # shape: (bs, mics_num, frames_num, emb_size)
        
        agent_position_emb = self.convert_position_to_emb(position=agent_position)
        # shape: (bs, agents_num, frames_num, emb_size)

        agent_look_direction_emb = self.convert_look_direction_to_emb(
        ...
        total_mag, total_cos, total_sin = self.wav_to_spectrogram_phase(total_waveform, eps)
        # (bs, 1, frames_num, freq_bins)

        mic_mag, mic_cos, mic_sin = self.wav_to_spectrogram_phase(mic_waveform, eps)
        ...
        # Transform mic signal.
        enc1_pool, enc1 = self.mic_signal_conv_block1(x)
        enc2_pool, enc2 = self.mic_signal_conv_block2(enc1_pool)
        enc3_pool, enc3 = self.mic_signal_conv_block3(enc2_pool)
        # enc3_pool: (bs, 128, T=38, F=32)
        
        mic_signal_feature = enc3_pool.permute(0, 2, 1, 3)  # (bs, T, C=128, F=32)
        mic_signal_feature = mic_signal_feature.flatten(2)  # (bs, T, C*F=4096)

        mic_signal_feature = F.leaky_relu_(
            self.mic_signal_fc1(mic_signal_feature), negative_slope=0.01,
        )  # shape: (bs, T, 1024)
        
        # Transform mic position and look direction embeddings.
        mic_position_feature = mic_position_emb.permute(0, 2, 1, 3).flatten(2)
        ...
        mic_look_direction_feature = mic_look_direction_emb.permute(0, 2, 1, 3).flatten(2)
        ...
        # Calculate agent position and look direction features.
        ...
        inter_emb = torch.cat((mic_feature, agent_position_feature, 
            agent_look_direction_feature), dim=-1)
        # (bs, agents_num, T, 1536)
